...
def _run_smoke(wrapper: str, binary: Path) -> None:
    with tempfile.TemporaryDirectory(prefix=f"kronroe-{wrapper}-wrapper-smoke-") as tmp:
        env = os.environ.copy()
        env["KRONROE_MCP_BIN"] = str(binary)
        env["KRONROE_MCP_DB_PATH"] = str(Path(tmp) / "wrapper-smoke.kronroe")

        if wrapper == "npm":
            cmd = ["node", "packages/kronroe-mcp/bin/kronroe-mcp.js"]
        elif wrapper == "python":
            cmd = ["python3", "-m", "kronroe_mcp.cli"]
